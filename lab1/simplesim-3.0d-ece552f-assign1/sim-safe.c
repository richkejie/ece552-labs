...
static counter_t num_1cyc_stalls_q1;
static counter_t num_1cyc_stalls_q2;

static counter_t num_2cyc_stalls_q1;
static counter_t num_2cyc_stalls_q2;

static counter_t num_other_stalls_q1;
static counter_t num_other_stalls_q2;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "num_1cyc_stalls_q1",
      "total number of 1 cycle stalls (q1)",
      &num_1cyc_stalls_q1, num_1cyc_stalls_q1, NULL);

  stat_reg_counter(sdb, "num_1cyc_stalls_q2",
      "total number of 1 cycle stalls (q2)",
      &num_1cyc_stalls_q2, num_1cyc_stalls_q2, NULL);

  stat_reg_counter(sdb, "num_2cyc_stalls_q1",
      "total number of 2 cycle stalls (q1)",
      &num_2cyc_stalls_q1, num_2cyc_stalls_q1, NULL);

  stat_reg_counter(sdb, "num_2cyc_stalls_q2",
      "total number of 2 cycle stalls (q2)",
      &num_2cyc_stalls_q2, num_2cyc_stalls_q2, NULL);

  stat_reg_counter(sdb, "num_other_stalls_q1",
      "total number of non 1/2 cycle stalls (q1)",
      &num_other_stalls_q1, num_other_stalls_q1, NULL);

  stat_reg_counter(sdb, "num_other_stalls_q2",
      "total number of non 1/2 cycle stalls (q2)",
      &num_other_stalls_q2, num_other_stalls_q2, NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   "CPI from RAW hazard (q1)",
		   "1 + (1/sim_num_insn) * (1*num_1cyc_stalls_q1 + 2*num_2cyc_stalls_q1)", NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "1 + (1/sim_num_insn) * (1*num_1cyc_stalls_q2 + 2*num_2cyc_stalls_q2)", NULL);
...
/* ECE552 Assignment 1 - BEGIN CODE */
/* this is where statistics are collected */
/* q1 */
{
    int i;
    // find input register with longest delay (stall needed)
    int delay_test;
    int delay_max = 0, max_delay_i = 0;
    for (i=0; i<3; i++) {
      if (r_in[i] == DNA) continue; // don't check registers that aren't actually accessed (DNA)
      delay_test = reg_ready_q1[r_in[i]] - sim_num_insn;
      if (delay_test > delay_max) {
        delay_max = delay_test;
        max_delay_i = i;
      }
    }
    int delay = delay_max;
    if (r_in[max_delay_i] != DNA && delay > 0) { /* if delay >= 0 then we have a stall */
      ...
      if (delay == 1) num_1cyc_stalls_q1++;
      else if (delay == 2) num_2cyc_stalls_q1++;
      else num_other_stalls_q1++;
      ...
      for (j=0; j<MD_TOTAL_REGS; j++) {
        if (reg_ready_q1[j] - delay < 0) reg_ready_q1[j] = 0;
        else reg_ready_q1[j] -= delay;
      }
    }
...
/* q2 */
{
    ...
    for (i=0; i<3; i++) {
      if (r_in[i] == DNA) continue; // don't check registers that aren't actually accessed (DNA)
      delay_test = reg_ready_q2[r_in[i]] - sim_num_insn;
      if (delay_test > delay_max) {
        delay_max = delay_test;
        max_delay_i = i;
      }
...
if (MD_OP_FLAGS(op) & F_ICOMP) {
  /* if instr is a int computation, since there are 2 EX stages
      we need to stall for 1 cycle
      i.e., value ready at end of EX2, can be used at start of EX1 for next instr */
...
if ((MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_LOAD)) {
  /* since 2 EX stages, loads cause immediate dependent instrs
      to stall for 2 cycles,
      i.e., value ready at end of M, can be used at start of EX1 for next instr */
  /* there is a case where a dependent instr 2 instrs away
      will stall for 1 cycle, but don't have to encode that here */
