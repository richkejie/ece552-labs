...
/*
	Case 1: Memory will jump two blocks at a time. This goes outside the next-line
            ...
            so should get ~100,000 dl1 cache misses here
// */
	asm("$L101:	sltu	$2,$3,$5");
	asm("beq	$2,$0,$L201");
	asm("lw		$4,0($3)");
...
/*
	Case 2: Memory will jump one blocks at a time. This is exactly the next block size
		the prefetcher fetches. This should succeed every time. We can also use any 			
        size below 64 (such as 32). As it's smaller than block size, it will be 		
        prefetched by next-line. This loop will run 200k times. 

        so should get ~0 dl1 cache misses here
*/
